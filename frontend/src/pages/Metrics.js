    ...
    <div className="fade-in">
      <div className="page-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
             <div className="stat-card" style={{ padding: '8px 20px', border: '2px solid var(--primary)', background: 'rgba(59, 130, 246, 0.05)' }}>
                <span style={{ fontSize: '12px', color: 'var(--gray-600)', textTransform: 'uppercase' }}>Overall AI Score</span>
                <div style={{ fontSize: '24px', fontWeight: '800', color: 'var(--primary)' }}>{(metrics.overallScore || 0).toFixed(0)}%</div>
             </div>
      ...
      <div className="grid">
        <div className="stat-card">
          ...
          <div className="stat-value">{metrics.sprintPredictability?.score || 0}%</div>
          <div className="stat-change" style={{ color: 'var(--gray-600)', fontSize: '12px' }}>
            {metrics.sprintPredictability?.message || 'Insufficent data'}
          </div>
        </div>
        ...
        <div className="stat-card">
          ...
          <div style={{ marginTop: '8px', marginBottom: '8px' }}>
            <span className={`risk-${metrics.riskDetectionTiming?.riskLevel?.toLowerCase() || 'low'}`}>
              {metrics.riskDetectionTiming?.riskLevel || 'Low'}
            </span>
          </div>
          <div className="stat-change" style={{ color: 'var(--gray-600)', fontSize: '12px' }}>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ color: 'var(--success)' }}>
            {metrics.velocityStability?.score || 0}%
          </div>
          <div className="stat-change" style={{ color: 'var(--gray-600)', fontSize: '12px' }}>
            {metrics.velocityStability?.message || 'N/A'}
          </div>
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ color: 'var(--info)' }}>
            {metrics.taskCompletionRate?.rate || 0}%
          </div>
          <div className="stat-change" style={{ color: 'var(--gray-600)', fontSize: '12px' }}>
            {metrics.taskCompletionRate?.message || 'N/A'}
          </div>
        </div>
      </div>
